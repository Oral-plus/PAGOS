...
function safeDateFormat($date, $format = 'd/m/Y') {
    ...
    if (is_object($date) && method_exists($date, 'format')) return $date->format($format);
    if (is_string($date)) {
        $t = strtotime($date);
...
for ($i=0;$i<10;$i++) {
    $cerradas[] = [
        ...
        'docnum_interno_sap' => 'SAP' . str_pad($i+1,4,'0',STR_PAD_LEFT),
        'codigo_sn' => 'NIT' . rand(1000,9999),
        'fecha_vencimiento' => date('Y-m-d', strtotime("-".($i*3).' days')),
        'saldo_pendiente' => rand(10000,100000)/100,
        'priority' => ($i%3==0?'alta':($i%3==1?'media':'baja')),
        'status' => 'C',
        'dias_finalizada' => rand(0,30),
        'final' => ($i%2==0?'si':''),
        'TipoDocumento' => 'FAC'
    ];
}
...
$egreso = [
    'facturas' => [],
    'numero_documento' => 'EGRESO-TEST',
    'total_valor' => 0
];
for ($i=0;$i<5;$i++) {
    $egreso['facturas'][] = [
        'Factura' => 'F' . ($i+1),
        'docnum_interno_sap' => 'SAP' . ($i+1),
        'FechaFactura' => date('Y-m-d', strtotime('-'.($i).' days')),
        'numero_factura_proveedor' => 'NF' . ($i+1),
        'ValorPagado' => rand(10000,50000)/100
    ];
    $egreso['total_valor'] += $egreso['facturas'][$i]['ValorPagado'];
}

// Recreate generatePDF logic (minimal)
...
if (!empty($egreso['facturas'])) {
    ...
    foreach ($egreso['facturas'] as $f) {
        if (in_array($f['Factura'],$seen)) continue;
        $seen[] = $f['Factura'];
    ...
    $pdf->Cell(40,8,number_format($egreso['total_valor'],3,'.',','),1,1,'R',true);
}

// Write to file
$out = __DIR__ . '/test_real_output.pdf';
$pdf->Output('F', $out);
if (file_exists($out)) {
...
// Print header/hex for inspection
$d = file_get_contents($out);
echo "HEAD: " . substr($d,0,16) . "\n";
for ($i=0;$i<min(128,strlen($d));$i++) {
    printf('%02X ', ord($d[$i]));
    if ((($i+1)%16)==0) echo "\n";
}
echo "\n";
